...
mysql -u $MYSQL_USER -p$MYSQL_PASSWD $MYSQL_DB --show-warnings --execute="LOAD DATA INFILE '/var/lib/mysql/traffic_details.csv' IGNORE INTO TABLE traffic_details_full FIELDS TERMINATED BY '\t' (ts,mac,remote_ip, outgoing,incoming);"
